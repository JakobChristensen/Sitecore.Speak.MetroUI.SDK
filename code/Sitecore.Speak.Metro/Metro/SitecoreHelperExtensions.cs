  ...
  public static class SitecoreHelperExtensions
  {
    ...
    public static HtmlString Menu([NotNull] this SitecoreHelper htmlHelper, [NotNull] IEnumerable<Item> children, [NotNull] string displayFieldName, [NotNull] string iconFieldName, [NotNull] string clickFieldName)
    {
      ...
      foreach (var item in children)
      {
        ...
        if (item.Is(MetroApplication.Separator))
        {
        ...
        {
          result.WriteLine("<li role=\"presentation\" class=\"menu-title\">" + item["Text"] + "</li>");
          continue;
        }
        ...
        if (item.HasChildren)
        {
          ...
          result.WriteLine("  <ul class=\"dropdown-menu\" data-role=\"dropdown\">");
    ...
    [NotNull]
    private static string GetIconTag([NotNull] Item item, [NotNull] string iconFieldName, [NotNull] string additionalClassName)
    {
      Debug.ArgumentNotNull(item, "item");
      Debug.ArgumentNotNull(iconFieldName, "iconFieldName");
      ...
      var size = 0;
      string result;
      switch (iconFieldName.ToLowerInvariant())
      {
        case "$icon":
      ...
      if (!result.StartsWith("icon-", StringComparison.InvariantCultureIgnoreCase))
      {
        result = "icon-" + result;
      }
      ...
      return string.Format("<i class=\"{0}\"></i>", result);
    }
